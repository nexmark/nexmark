...
public final class NexmarkGlobalConfiguration {
	...
	public static final String NEXMARK_CONF_FILENAME = "nexmark.yaml";

	// --------------------------------------------------------------------------------------------

	private NexmarkGlobalConfiguration() {}
	...
	public static Configuration loadConfiguration(final String configDir, @Nullable final Configuration dynamicProperties) {
		...
		// get Nexmark yaml configuration file
		final File yamlConfigFile = new File(confDirFile, NEXMARK_CONF_FILENAME);

		if (!yamlConfigFile.exists()) {
			throw new IllegalConfigurationException(
				"The Nexmark config file '" + yamlConfigFile +
